...
def load_data():
    ...
    if os.path.exists(file):
        df = pd.read_csv(file)
        # Unified Naming Logic to avoid KeyErrors across different export versions
        ...
        # Ensure Baseline comparison column exists
        if 'Base_Prob_Pct' not in df.columns:
            df['Base_Prob_Pct'] = df.get('New_Prob_Pct', 50) - 5
        return df
    return None
...
def get_comprehensive_intel(country, c_data, custom_roi):
    intel = {
        "Germany": (
            ...
            f"**Strategic Verdict (ROI {custom_roi:.1f}):** AI Confidence is {c_data.get('New_Prob_Pct', 0):.1f}%. The model identifies this as a structurally strong market currently undergoing a high-volatility transition."
        ),
        "India": (
            ...
            f"**Strategic Verdict (ROI {custom_roi:.1f}):** High market headroom. The model identifies this as a strategic entry point where long-term fundamentals outstrip short-term policy headwinds."
        )
    }
    ...
    if country not in intel:
        ...
        dyn_context = f"**Market Dynamics:** {country} exhibits a Risk-Adjusted Confidence of {c_data.get('New_Prob_Pct', 0):.1f}%. The Opportunity Gap stands at {gap:.2f}, indicating untapped potential versus structural risk."
        dyn_roi = f"**Strategic Verdict (ROI {custom_roi:.1f}):** The AI has weighed current purchasing power against infrastructure saturation. Auditor discretion is advised relative to the 78% 'Chaos Regime' benchmark."
...
def show_final_report(country, w_safe, w_room, w_wealth):
    ...
    # Accurate ROI Re-calculation based on User-defined weights
...
# --- 5. SINGLE-PAGE LAYOUT (THE WORLD MAP) ---
...
with col_panel:
    ...
    if selected_country:
        ...
        c1, c2 = st.columns(2)
        c1.metric("ROI Score", f"{c_data.get('ROI_Score', 0):.1f}")
        c2.metric("AI Confidence", f"{c_data.get('New_Prob_Pct', 0):.1f}%")
        ...
        st.markdown("<h3 style='margin-top: 0; color: #1e293b;'>🌍 Portfolio Audit</h3>", unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        ...
        st.info("👆 **Select a market on the map** to run the structural risk audit.")
